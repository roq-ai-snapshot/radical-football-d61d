...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createPlayer() {
    ...
    if (body?.player_exercise?.length > 0) {
      const create_player_exercise = body.player_exercise;
      body.player_exercise = {
        create: create_player_exercise,
      };
    } else {
      delete body.player_exercise;
    }
